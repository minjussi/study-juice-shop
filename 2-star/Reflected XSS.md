...
- url에 파라미터로 값을 넘길 때는 (<a href="http://site.com?search= ">) url 인코딩으로 방어할 수 있다. 그러나 href 전체 입력을 사용자 입력으로 받으면, 이는 url 인코딩으로 방어할 수 없게 된다. 이때는 url 전체가 안전한 값인지를 검증하는 과정이 필요해지므로 href 전체를 사용자 입력으로 받는 것은 지양해야 한다. 
